...
- **Coolblue list→detail (2026-09-18, offline, three gated slices; no live 03):** live repeats `110505Z`/`114603Z`/`123057Z` all `MAX_ACQUISITION_STEPS` `detail_link=NO_URL`. Not a miss of #10 (surface already `list_results`) and not the 05 allowlist (no product href on candidates). (A `06f7f00`) `_snapshot` 400k cap counted `<script>`/`<style>`/`<head>` — `html_chars=699128`, disk 200k still inside a stylesheet; `prepare_html_for_snapshot` strips those *then* keeps `<body>` *then* caps. (B `ef3b066`) `repeating_only` took the first repeating group (Language/Account/Wishlist); leaf replaced text whenever `html_cands` was non-empty. Now pick max `n × (D2c + text-link)`; replace text only if a card is itemish (price **or** href+digit-runs+≥2 lines — arXiv has no €). (C `64a4f88`) `inject_preferred_action_affordances` puts shown-candidate `primary_action` hrefs into the cap *before* `panel_option` filters (same source as the #24 allowlist; no `/product/` lexicon). Offline: `evals/html_cap_body/`, `evals/html_leaf_list/`, `evals/preferred_href_allowlist/`. **Does not close #24b until live 03 (user OK, 2–3 repeats).** Not html_b2. Not vision. Not network interception.

...
**H-leak:** FIX 1 `82cae84` Open #29; FIX 2 `2f030e3` entity-geen-claim. Open #30 2a same-host (`e2229bc`). Open #31 overlay-dismiss (offline). Open #32 HTML-replace splice (offline). **Packager-exclusiviteit** = `MECHANISM_NOTES` §17 (toets vóór nieuwe `return pool_X`). Path B geïnventariseerd (§14), niet gefixt. Middag-queue — **niet starten** zonder `ja, start` + tmux-sessienaam.

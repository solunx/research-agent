...
**Laatste commit:** `6debab8` — Open #10 D2c herijking. Live `070449Z`: `stop_reason=CONTRACT_SATISFIED` `claim_extracted=EXTRACTED` abs=`live_offer_state`. **#26 bind gezien; unbind/switch niet.** Docker: altijd `docker compose build` vóór run.

**Open (top 3):**
1. **#26** — bind live (`062211Z`/`070449Z`); unbind/switch nog niet. Open laten.
2. **#10** — deze trigger live-bewezen (`070449Z`); T=3 + D2c-tighten blijft provisionally, niet gelockt over alle paginatypes.
3. **#24b** — leaf-html op search live; allowlist (`#24`) live-bewezen. Niet `html_b2`.

...
**Laatste commit:** Open #24b Fase 2.1 raw-HTML plumbing (nog niet gewired). Fase 1 identity: `cc1871e`. **#26 OPEN.** Docker: altijd `docker compose build` vóór run.

**Open (top 3):**
1. **#26** — unbind/switch live nooit gezien (`083112Z` hypothese; `093236Z`/`102344Z`/`111714Z` geen event). Offline groen. Open laten.
2. **#24b** — Fase 1 live `162616Z`: meerdere pdf-hrefs. Chunking nog open. Fase 2.1 plumbing in code; leaf-html nog niet additief. Niet `html_b2`.
3. **#10** — abs `price_hits=4` → `list_results`. **#28** PDF-catch in code; live 05 na OK.

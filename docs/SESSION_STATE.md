...
**Laatste commit:** `cae4402` — Open #26. **#27 in tree:** wrap innerText-regels `len>240` + append één long-text candidate; offline `evals/long_line_units/test_long_line_units_offline_v0.py`. Live 05 `20260917T093236Z`: `claim_extracted=NOT_VISIBLE` terwijl abstract in page_text stond; `subject_instance=RELEVANT`; #26 niet live bewezen (geen unbind). Docker: altijd `docker compose build` vóór run.

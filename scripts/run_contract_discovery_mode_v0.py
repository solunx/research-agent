...
def main() -> int:
    ...
    if mode == "CD0" and surfaces:
        # compare provisional-style contract to a heuristic CD1 for offline signal
    ...
    payload: dict[str, Any] = {
        ...
        "success_v0": bool(validation.ok and (gaps.contract_explains_run or mode == "CD0")),
        "go_no_go": {
            "go": bool(validation.ok),
            "reasons": (
                ["validation.ok"]
                if validation.ok
                else [f"validation errors: {validation.errors[:3]}"]
            ),
        },
        "note": (
    ...
    print(f"mode={mode} task={task_id} source={contract.get('_source')} llm_calls={payload['llm_calls']}")
    print(f"validation.ok={validation.ok} explains_run={gaps.contract_explains_run} duration={dur:.1f}s")
    ...
    print(f"wrote {out}")
    return 0 if payload["go_no_go"]["go"] else 1
